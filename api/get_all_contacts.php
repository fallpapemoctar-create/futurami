...
// Liste globale des contacts, toutes sociétés confondues (llx_socpeople x llx_societe),
// utilisée par l'onglet "Contacts" de TiersPage. Contrairement à get_contacts.php
// (qui exige un client_id pour l'onglet "détail société"), cet endpoint ne filtre pas
// par société : il sert la vue transverse.
try {
    ...
    if ($q !== '') {
        $sql .= " AND (p.firstname LIKE :search OR p.lastname LIKE :search OR p.email LIKE :search OR p.phone LIKE :search
                        OR p.phone_perso LIKE :search OR p.phone_mobile LIKE :search OR p.fax LIKE :search OR p.poste LIKE :search
